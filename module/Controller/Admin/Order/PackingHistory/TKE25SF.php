...
class DownloadTkeReleaseController extends \Controller\Admin\Controller{
    ...
    public function makeExcel($refineList, $div){
        $goodsTitle1 = 'TKE 춘추점퍼(25/TKEK)';

        $totalCnt = 31;
        $addCnt = 2;
        $dpCnt = $totalCnt + $addCnt;

        $fileTitle = "TKE춘추출고패킹리스트_".date('Y-m-d');
    ...
    public function getList($teeCode1){
        ...
        $sql = "
            ...
            join es_memberGroup f
              on e.groupSno = f.sno
            where 1=1
            and a.orderStatus in ('p3')
            and a.goodsNo in (
            '{$teeCode1}'
        ) order by concat(receiverAddress,receiverAddressSub), receiverName";
