...
<form id="frmSearchBase" method="get" class="content-form js-search-form js-form-enter-submit">
    ...
    <?php include('scm_stock_monthly_list_search2.php'); ?>
</form>
            ...
            if (gd_isset($data)) {
                foreach ($data as $val) {
                    ...
                    <tr>
                        ...
                        <?php foreach($val['optionList'] as $optionData) { ?>
                        <td class="center">
                            <a href="<?=$monthlyValue[$optionData['optionNo']]['detailLink']?>" target="_blank"><?=number_format($monthlyValue[$optionData['optionNo']]['stockIn'])?></a>
                        </td>
                        <?php } ?>
                        <?php if ( $maxOptionCnt > count($val['optionList']) ) { ?>
                            <?php for( $forIdx=0; $forIdx < ($maxOptionCnt-count($val['optionList'])); $forIdx++ ) { ?>
                    ...
                    <?php } ?>
                    <tr>
                        <td class="center bg-light-yellow" >기간합계</td>
                        <?php foreach($val['optionList'] as $optionData) { ?>
                    ...
                    <tr>
                        <td class="center bg-light-yellow" >기간비율</td>
                        <?php foreach($val['optionList'] as $optionData) { ?>
                            <td class="center bg-light-yellow" >
