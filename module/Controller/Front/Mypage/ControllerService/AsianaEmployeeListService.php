 ...
 */
class AsianaEmployeeListService implements ListInterface {
    ...
    public function getList($searchData): array {
        ...
        if(!empty($request['simple_excel_download'])){
            $searchData['page'] = 1;
            $searchData['pageNum'] = 10000;
        }
        $data = $this->getTraitList($searchData, 'getList');
        $data['newPage'] = clone  $data['page'];
    ...
    public function setRefineEachListData($each, $key, &$mixData){
        //gd_debug($mixData['page']);
        $each['no'] = $mixData['newPage']->idx--;
        $orderHistory = array_reverse(json_decode($each['provideInfo'], true));
        $aggData = [];
        foreach($orderHistory as $order){
            $year = substr($order['requestDt'],2,2);
            $aggData[$year][$order['prdName']] += $order['orderCnt'];
        }
        //gd_debug();
        $orderAgg = [];
        foreach($aggData as $aggYear => $aggEach){
            foreach($aggEach as $prdName => $orderCnt){
                $orderAgg[$aggYear][] = $prdName . '*' . $orderCnt;
            }
        }
        $each['orderAgg'] = $orderAgg; //집계
        $each['orderProvide'] = json_decode($each['orderHistory'],true);
        $each['orderHistory'] = json_encode($orderHistory);

        if(empty($each['orderHistory'])){
            $each['orderHistory'] = [];
        }
