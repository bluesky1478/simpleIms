        ...
        <table class="table table-cols">
            ...
            <tr>
                ...
                <td colspan="3">
                    ...
                    <label class="radio-inline">
                        <input type="radio" name="scmFl" value="1" <?=gd_isset($checked['scmFl']['1']); ?> onclick="layer_register('scm', 'checkbox')"/>
                    </label>
                    <label>
                        <button type="button" class="btn btn-sm btn-gray" onclick="layer_register('scmOrder','checkbox')" >고객사 선택</button>
                    </label>
            ...
            <tr>
                ...
                <td>
                    <label class="radio-inline">
                        <input type="radio" name="userConsultMemo" value=""   <?=gd_isset($checked['userConsultMemo']['']); ?> />전체
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="userConsultMemo" value="1" <?=gd_isset($checked['userConsultMemo']['2']); ?> />일반주문
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="userConsultMemo" value="2" <?=gd_isset($checked['userConsultMemo']['1']); ?> />추가결제
                    </label>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <th>한국타이어<br>본사 주문건</th>
                <td >
                    <label class="radio-inline">
                        <input type="radio" name="hankookMasterFl" value="all" <?= gd_isset($checked['hankookMasterFl']['all']); ?>/>전체
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="hankookMasterFl" value="1" <?= gd_isset($checked['hankookMasterFl'][1]); ?>/>한국타이어 본사 주문
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="hankookMasterFl" value="2" <?= gd_isset($checked['hankookMasterFl'][2]); ?>/>한국타이어 본사 주문 제외
                    </label>
                </td>
            </tr>
            <tr>
                ...
                <th>한국타이어<br>본사 주문 구분</th>
                <td >
                    <label class="radio-inline">
                        <input type="radio" name="hankookMasterOpenFl" value="all" <?= gd_isset($checked['hankookMasterOpenFl']['all']); ?>/>전체
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="hankookMasterOpenFl" value="0" <?= gd_isset($checked['hankookMasterOpenFl'][0]); ?>/>오픈패키지
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="hankookMasterOpenFl" value="1" <?= gd_isset($checked['hankookMasterOpenFl'][1]); ?>/>재계약
                    </label>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    ...
                    <label class="radio-inline">
                        <input type="radio" name="userConsultMemo" value="2" <?=gd_isset($checked['userConsultMemo']['2']); ?> />일반주문
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="userConsultMemo" value="1" <?=gd_isset($checked['userConsultMemo']['1']); ?> />추가결제
                    </label>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <div class="form-inline">
                        배송요청일 <span class="text-muted">(~지정일까지)</span> :
                        <div class="input-group js-datepicker">
                            <input type="text" name="reqDeliveryDt" value="<?= $search['reqDeliveryDt']; ?>" class="form-control width-xs" placeholder="">
                            <span class="input-group-addon">
                                <span class="btn-icon-calendar">
                                </span>
                            </span>
                    ...
                    <div>
                        <label class="checkbox-inline">
                            <input type="checkbox" name="isAcceptOnly" value="y" <?= gd_isset($checked['isAcceptOnly']['y']); ?>> 승인완료/승인없음 건
                        </label>
                    </div>
                    <div>
                        <label class="checkbox-inline">
                            <input type="checkbox" name="isOrderOnly" value="y" <?= gd_isset($checked['isOrderOnly']['y']); ?>> 교환제외
                        </label>
                    </div>
                </td>
            </tr>
...
<!--                            </label>-->
    ...
    <div class="table-header <?=$tableHeaderClass?>">
        <div class="pull-left">
            ...
            / 수량 :<span class="text-danger"><?=number_format($page->recode['orderGoodsCnt'])?></span>개
...
<script type="text/javascript">
    //튜닝 스크립트
    $(function(){
        $('#memberBranch').change(function(){

            if( $.isEmpty($('#memberBranch').val()) ){
                return false;
            }

            var memberBranchName = $('#memberBranch').val();
            var params = {
                mode : 'getBranchDept',
                memberBranchName : memberBranchName
            };
            $.post('claim_ps.php', params, function(jsonResult){
                $('#branchDept').html('<option value="">=== 부서선택  ===</option>');
                if( !$.isEmpty(jsonResult) ){
                    for(var idx in jsonResult.data.selectData) {
                        var selected = '<?=$search['branchDept']?>' == idx ? 'selected' : '';
                        var htmlTag = '<option value="'+idx+'" '+selected+' >'+jsonResult.data.selectData[idx]+'</option>';
                        $('#branchDept').append(htmlTag);
                    }
                }
            });
        });
