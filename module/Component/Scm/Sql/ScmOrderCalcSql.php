...
/*        $table['g'] = new TableVo('sl_orderScm','tableOrderScm','g');
        ...
        if( !empty($searchData['scmFl']) && 'all' !== $searchData['scmFl']  ){
            if( 'n' === $searchData['scmFl']){
                //본사
                $searchVo->setWhere('c.scmNo = ?');
                $searchVo->setWhereValue('1');
            }else{
                //공급사 검색
                $searchVo->setWhere(DBUtil::bind('c.scmNo', DBUtil::IN, count($searchData['scmNo']) ));
    ...
    public function getGoodsList($orderNo){
