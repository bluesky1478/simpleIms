                ...
                foreach ($data as $val) {
                    ...
                    <tr class="center">
                        ...
                        <td>
                            ...
                            <?php if ( '2022-07-01 00:00:00' >= $val['entryDt'] ) { ?>
                                <button type="button" class="btn btn-red btn-sm btnWakeForce" data-no="<?= $val['sleepNo']; ?>">강제해제</button>
                            <?php } ?>
                        </td>
                    </tr>
                    <?php
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('.btnWakeForce', $formList).on('click', function (e) {

            console.log('==> btnWakeForce');

            BootstrapDialog.confirm({
                title: "선택휴면해제",
                message: "선택한 회원을 휴면회원 상태에서 강제 해제하시겠습니까? 해제 시 해당 회원은 다시 서비스를 정상적으로 이용하실 수 있습니다.",
                btnOKLabel: "강제해제",
                callback: function (result) {
                    if (result) {
                        var data = $formList.serializeArray();
                        data.push({name: "mode", value: "wake_member_force"});
                        data.push({name: "sleepNo", value: member.get_member_attribute(e)});
                        $.ajax('../member/custom_member_sleep_ps.php', {
                            type: "post",
                            data: data
                        }).done(function(data) {
