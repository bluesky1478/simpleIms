 ...
 */
class LayerOrderAsRegistController extends \Bundle\Controller\Front\Mypage\LayerOrderExchangeRegistController
{
    /**
     * AS 처리
     * {@inheritdoc}
    ...
    public function index()
    {
        try {
            ...
            //$orderData = $order->getOrderClaimList($orderData, 'exchangeRegist');
            // 사용자 반품/교환/환불 신청 데이터 생성
            $orderData = $order->getOrderClaimList($orderData, 'exchange');
            $orderService = SlLoader::cLoad('Order', 'OrderService');
            $orderData = $orderService->refineOrderList($orderData);
            //gd_debug($orderData);
