...
<section id="imsApp">
    <form id="frm">
        <div class="page-header js-affix" style="margin-bottom:0 !important; vertical-align: bottom">
            <h3 class="">입고 등록 <span class="font-13 text-danger">* : 필수입력</span></h3>
...
<script type="text/javascript">
    ...
    $(()=>{
        ...
        ImsBoneService.setMounted(serviceData, ()=>{
            <?php if ($chooseFabricSno != 0) { ?>
            vueApp.StoredFabric.sno = <?=$chooseFabricSno?>;
            <?php } ?>
            let oDate = new Date();
            vueApp.StoredFabricInput.inputDt = oDate.getFullYear() + '-' + String(oDate.getMonth() + 1).padStart(2, '0') + '-' + String(oDate.getDate()).padStart(2, '0');
        });
        ...
        ImsBoneService.setMethod(serviceData,{
            ...
            save : ()=>{
                ...
                $.imsPost('saveStoredFabricInput', oSendData).then((data)=>{
                    $.imsPostAfter(data,(data)=>{
                        $.msg('입고 등록 완료','','success').then(()=>{
