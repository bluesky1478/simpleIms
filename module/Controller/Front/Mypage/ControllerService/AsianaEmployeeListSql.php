 ...
 */
namespace Controller\Front\Mypage\ControllerService;
 ...
 * Inout 리스트 SQL
 */
class AsianaEmployeeListSql {

    use ListSqlTrait;

    const MAIN_TABLE = 'sl_asianaEmployee';
    ...
    public function setCondition($searchData, $searchVo){
        ...
        if( 'all' !== $searchData['retiredFl'] ){
            $searchVo->setWhere( 'a.retiredFl = ?');
            $searchVo->setWhereValue( $searchData['retiredFl']  );
        }

        if( 'y' !== $searchData['includeRetired'] ){
            $searchVo->setWhere( 'a.retiredFl = ?');
            $searchVo->setWhereValue('n');
        }

        if( '' !== $searchData['empTeam'] && !empty($searchData['empTeam']) ){
            $searchVo->setWhere( 'a.empTeam = ?');
            $searchVo->setWhereValue( $searchData['empTeam']  );
        }
        if( '' !== $searchData['empPart1'] && !empty($searchData['empPart1'])  ){
            $searchVo->setWhere( 'a.empPart1 = ?');
            $searchVo->setWhereValue( $searchData['empPart1']  );
        }
        if( '' !== $searchData['empPart2'] && !empty($searchData['empPart2']) ){
            $searchVo->setWhere( 'a.empPart2 = ?');
            $searchVo->setWhereValue( $searchData['empPart2']  );
        }

        //$searchVo->setWhere('a.scmNo = ?');
        //$searchVo->setWhereValue('2');
