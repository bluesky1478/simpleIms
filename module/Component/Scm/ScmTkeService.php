...
class ScmTkeService {
    ...
    public static function getPreOrderGoodsOption(){
        return [
            //TS 동계 카라티
            //6632, //95 중단
            //3195, //100 중단
    ...
    public function setTkePreOrderStatus(){
        ...
        $orderGoodsList = DBUtil2::getList(DB_ORDER_GOODS, new SearchVo("regDt >= '{$checkDt}' and orderStatus=?", 'p1')); //특정 일자 이후 체크
        ...
        foreach($orderGoodsList as $orderGoodsData){
            ...
            if( in_array($orderGoodsData['goodsNo'] , ScmTkeService::getPreOrderGoods()) ){
                ...
                DBUtil2::update(DB_ORDER_GOODS, ["orderStatus = '{$changeOrderStatus}' "], new SearchVo(" regDt >= '{$checkDt}' and sno=?", $orderGoodsData['sno']));
            }
        }
    ...
    public function saveMemberTkeEach($paramData, $key, &$mixData){
        ...
        //$beforeMemberTempTable = "zzz_member_240307";
        //$beforeMemberTempTable = "zzz_es_member_240426";
        //$beforeMemberTempTable = "zzz_es_member_241010";
        $beforeMemberTempTable = "stmp_member_250523";
        //$beforeMemberTempTable = "zzz_tkeMember_250916";
        ...
        // select * from zzz_tkeMember_250916 where memId='10212763'
...
/*        if(!is_object($goodsBenefit)){
    ...
    public function changePassword(){
        //TKE암호 변경
        $list = DBUtil2::runSelect("select * from es_member where memId in (
            's031-709-8571'
            ,'s02-2113-8571'
            ,'s02-711-1533'
            ,'s02-935-5383'
        ...
        foreach($list as $each){
            $pwdStr = 'a' . preg_replace("/[^0-9]*/s", "", $each['memId']); //정직원처럼 (사번일경우)
            $password = Digester::digest($pwdStr);
            $rslt = DBUtil2::update(DB_MEMBER, [
                'memPw' => $password
            ], new SearchVo('memNo=?', $each['memNo']));
            gd_debug( $each['memNm'] . ' : ' . $rslt );
        }
    ...
    /**
     *  TKE 전체 주소지 변경이 있었어서
     *  새로운 주소에 맞게 리파인 했던 작업이다. 25-04-03
     * @throws Exception
     */
    public static function manualDeliveryRefine(){
        $sql = "select a.orderNo 
            from es_order a 
                join es_orderInfo b on a.orderNo = b.orderNo 
                join es_orderGoods c on a.orderNo = c.orderNo 
            where c.orderStatus = 'p3'
            and b.receiverAddress not like '서울특별시 마포구 월드컵북로54길 25%' 
            and c.goodsNo in (1000000559, 1000000560)
            and c.scmNo = 8";

        $list = DBUtil2::runSelect($sql);

        foreach($list as $each){
            $orderScmData = DBUtil2::getOne('sl_orderScm', 'orderNo', $each['orderNo']);

            $sql2 = "select subject from zzzz_sl_setScmDeliveryList_250324 where sno = {$orderScmData['scmDeliverySno']} ";
            $beforeSubject = DBUtil2::runSelect($sql2)[0]['subject'];

            if(!empty($beforeSubject)){
                $deliveryData = DBUtil2::getOne('sl_setScmDeliveryList','subject', $beforeSubject); //새로운
                if(!empty($deliveryData)){
                    DBUtil2::update('sl_orderScm',['scmDeliverySno'=>$deliveryData['sno']],new SearchVo('sno=?',$orderScmData['sno']));
                }
                /*DBUtil2::update(DB_ORDER_INFO, [
                    'receiverName' => trim($deliveryData['receiverName']),
                    'receiverCellPhone' => trim($deliveryData['receiverCellPhone']),
                    'receiverAddress' => trim($deliveryData['receiverAddress']),
                    'receiverAddressSub' => trim($deliveryData['receiverAddressSub']),
                    'receiverZonecode' => trim($deliveryData['receiverZonecode']),
                ], new SearchVo('orderNo=?', $each['orderNo']));*/

                //SitelabLogger::logger2(__METHOD__, $each['orderNo']);
            }
        }
