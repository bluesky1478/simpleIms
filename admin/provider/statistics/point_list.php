...
<form id="form_search" method="get" class="content-form js-search-form js-form-enter-submit">
    ...
    <div class="page-header js-affix">
        ...
        <div class="btn-group"><input type="button" value="포인트 지급/차감" class="btn btn-red-line btn-register"/></div>
    </div>
    <div class="form-inline search-detail-box">
        <input type="hidden" name="detailSearch" value="<?= gd_isset($search['detailSearch']); ?>"/>
        <div class="table-title ">포인트 지급/차감내역 검색</div>
...
<form id="frmList" action="" method="get" target="ifrmProcess">
    ...
    <table class="table table-rows">
        ...
        <thead>
            <tr>
                <th>번호</th><th>아이디</th><th>이름</th><th>등급</th><th>지급액</th><th>차감액</th><th>잔여</th><th>지급/차감일</th><th>소멸예정일</th><th>처리자</th><th>사유</th>
            </tr>
        </thead>
        <tbody>
        <?php
        if (isset($data) && count($data) > 0) {
            ...
            foreach ($data as $val) {
                ...
                switch ($val['reasonCd']) {
                    ...
                    case '01005505':
                        ...
                        if (empty($val['handleCd']) === false) {
                            ...
                            $reasonContents .= '<span class="sl-blue" data-order-no="' . $val['handleCd'] . '">' . $val['handleCd'] . '</span>)';
                        }
                        break;
                    case '01005006':
                    case '01005007':
                ...
                <tr class="text-center" data-member-no="<?= $val['memNo']; ?>">
                    ...
                    <td>
                        <span class="font-eng js-layer-crm hand"><?= $memberMasking->masking('member','id',$val['memId']); ?></span>
                    </td>
                    <td>
                        <span class="" >
                    ...
                    <td class="font-num sl-blue" style="color:#2b50f0">
                        <?= $isMinusSign ? '-' : '(+)' . gd_money_format($val['mileage']) . gd_display_mileage_unit() ?></td>
                    <td class="font-num text-danger">
                        <?= $isMinusSign ? '(-)' . gd_money_format(substr($val['mileage'], 1)) . gd_display_mileage_unit() : '-' ?></td>
                    <td class="font-num">
                        <?= gd_money_format(substr($val['afterMileage'], 1)) . gd_display_mileage_unit() ?></td>
