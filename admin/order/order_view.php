...
<form id="frmOrder" name="frmOrder" action="./order_ps.php" method="post">
    ...
    <span class="label <?=$data['orderAcctClass']?>" style="font-size:30px;font-weight:bold;"><?=$data['orderAcctStr']?></span>
        ...
        <div class="tab-content loading">
            <div class="table-action" style="margin-bottom: 0px !important;">
                ...
                <div class="pull-left form-inline" style="height: 26px;">
                    <button type="button" class="btn btn-sm btn-white btn-sync-status" data-orderno="<?= $data['orderNo']; ?>">주문상태 갱신</button>
                ...
                <div class="pull-left form-inline" style="height: 26px;">
                    총수량: <?=number_format($totalGoodsCnt)?>
                </div>
                <div class="pull-left form-inline" style="height: 26px;">
                    상품금액: <?=number_format($totalGoodsPrice)?>
                </div>
                <div class="pull-left form-inline" style="height: 26px;">
                    총상품금액: <?=number_format($totalGoodsAmount)?>
                </div>
            ...
            <table class="table table-cols">
                ...
                <tr>
                    ...
                    <td class="input_area note">
                        <?php foreach( $orderFileList as $orderFileInfo ){ ?>
                            <div>* <a href="orderDownload.php?sno=<?=$orderFileInfo['sno']?>>" ><?=$orderFileInfo['fileName']?></a></div>
                        <?php } ?>
                    </td>
                </tr>
            </table>
    ...
    <div class="table-title gd-help-manual" style="display: none">
        <?php if (!$isProvider) { ?>취소/<?php } ?>교환/반품/환불 정보
    </div>

    <div id="tabClaimStatus" style="display: none">
        <ul class="nav nav-tabs mgb30" role="tablist">
            <?php if (!$isProvider) { ?>
    ...
    <div class="row">
                ...
                <table class="table table-cols">
                    ...
                    <tr <?php echo $styleDisplayNone; ?>>
                        ...
                        <td class="form-inline">
                            ...
                            <?php foreach ($data['goods'] as $sKey => $sVal) {
                                foreach ($sVal as $dKey => $dVal) {
                                    foreach ($dVal as $key => $val) { ?>
                                        <?php if(20 == $val['scmNo']){ ?>
                                        <div class="btn btn-white btn-han-tax-refine">계산서정제</div>
                                        <?php } ?>
                            <?php break; }}} ?>
        ...
        <div class="col-xs-12">
            <div class="table-title gd-help-manual">
                ...
                <div class="flo-right ">
                    <!--미결제금액 : <span class="text-danger" id="pay-history-not-payed">0</span>원-->
                    <span style="font-size:14px;font-weight: normal; color:#8C8C8C">
                        <button type="button" class="btn btn-red btn-sm js-paymentBtn" data-orderno="<?=$data['orderNo']?>" >결제추가</button>
                    </span>
                </div>
            </div>
            <table class="table table-cols">
                ...
                <tr>
                    <th class="text-center">결제요청일</th>
                    <th class="text-center">결제일자</th>
                    <th class="text-center">결제요청금액</th>
                    <th class="text-center">결제금액</th>
                    <th class="text-center">요청제목/결제번호</th>
                    <th class="text-center">결제상태</th>
                    <th class="text-center">결제형태</th>
                    <th class="text-center">영수형태</th>
                    <th class="text-center">삭제</th>
                </tr>
                <?php if( empty( $paymentsHistory) ) { ?>
                    <tr>
                        <td colspan="20" class="text-center">데이터가 없습니다.</td>
                    </tr>
                <?php }else{  ?>
                    <?php foreach($paymentsHistory as $paymentsData) { ?>
                        <tr>
                            <td class="text-center"><?=$paymentsData['regDt']?></td>
                            <td class="text-center"><?=$paymentsData['paymentDt']?></td>
                            <td class="text-center"><?=number_format($paymentsData['reqPrice'])?>원</td>
                            <td class="text-center">
                                <div><?=number_format($paymentsData['settlePrice'])?>원</div>
                                <?php if($paymentsData['useMileage'] > 0 ) { ?>
                                    <small>마일리지 : <?=number_format($paymentsData['useMileage'])?>사용</small>
                                <?php } ?>
                                <?php if($paymentsData['useDeposit'] > 0 ) { ?>
                                    <small>예치금 : <?=number_format($paymentsData['useDeposit'])?>사용</small>
                                <?php } ?>
                                <?php if($paymentsData['totalCoupon'] > 0 ) { ?>
                                    <small>쿠폰 : <?=number_format($paymentsData['totalCoupon'])?>사용</small>
                                <?php } ?>
                            </td>
                            <td  class="text-center">
                                <div><?=$paymentsData['paymentSubject']?></div>
                                <div>
                                    <div class="inline-block">
                                        <?php if (empty($paymentsData['collectOrderNo'])) { ?>
                                            <small class="text-muted">
                                                추가 결제에 대한 결제주문서가 없습니다.
                                            </small>
                                        <?php }else{ ?>
                                            <small >
                                                결제번호 : <a href="#;" class="text-info" onclick="open_order_link('<?=$paymentsData['collectOrderNo']?>', 'newTab', '')" title="주문번호" class="font-num" data-order-no="<?=$paymentsData['collectOrderNo']?>" data-is-provider="false"><?=$paymentsData['collectOrderNo']?></a>
                                            </small>
                                        <?php } ?>
                                    </div>
                                    ...
                                    <div class="inline-block form-inline">
                                        <input type="text" class="form-control width-md pay-number" />
                                        <input type="button" value="수정" class="btn btn-white btn-sm mod-pay-number-btn"  data-historysno="<?=$paymentsData['sno']?>" />
                                    </div>
                                    -->
                                </div>
                            </td>
                            <td  class="text-center">
                                <small><?=$paymentsData['orderStatusStr']?></small>
                            </td>
                            <td  class="text-center">
                                <small><?=$paymentsData['settleKindStr']?></small>
                            </td>
                            <td  class="text-center">
                                <small><?=$paymentsData['receiptKindStr']?></small>
                            </td>
                            <td  class="text-center">
                                <?php if(  'y' == $paymentsData['delFl'] ) { ?>
                                    <span class="text-danger">삭제완료</span>
                                <?php } else if( empty($paymentsData['collectOrderNo']) || 'p1' != $paymentsData['orderStatus'] ){  ?>
                                    <a class="btn btn-sm btn-gray js-deletePaymentBtn" data-sno="<?=$paymentsData['sno']?>" data-goodsno="<?=$paymentsData['goodsNo']?>"  >삭제</a>
                                <?php } else {  ?>
                                    삭제불가(결제완료)
                                <?php }  ?>
                            </td>
...
<script>
    ...
    $(function(){
        ...
        $('#select-delivery-list').on('change',function(){
            var deliveryInfo = $(this).val().split('▶');
            $('#receiverZonecode').val(deliveryInfo[0]);
            $('#receiverZipcode').val('');
            $('#receiverZipcodeText').text('');
            $('#receiverAddress').val(deliveryInfo[1]);
            $('#receiverAddressSub').val(deliveryInfo[2]);
        });

        $('.btn-han-tax-refine').click(()=>{
            $.post('./order_change_ps.php', {
                'mode' : 'set_refine_han_tax',
                'orderNo' : '<?= gd_isset($data['orderNo']); ?>',
            }, function (data) {
                location.reload();
            });
