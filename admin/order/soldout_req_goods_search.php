    ...
    <table class="table table-cols">
        ...
        <tbody>
                    ...
                    <?php if ($search['scmFl'] == 'y') {
                        foreach ($search['scmNo'] as $k => $v) { ?>
                            <span id="info_scm_<?= $v ?>" class="btn-group btn-group-xs">
                                <input type="hidden" name="scmNo[]" class="check-scm" value="<?= $v ?>"/>
            ...
            <tr>
                ...
                <th>발송여부</th>
                <td>
                    <label class="radio-inline"><input type="radio" name="sendType" value="all" <?=gd_isset($checked['sendType']['all']); ?> />전체</label>
                    <label class="radio-inline"><input type="radio" name="sendType" value="0" <?=gd_isset($checked['sendType']['0']); ?> />미발송</label>
                    <label class="radio-inline"><input type="radio" name="sendType" value="1" <?=gd_isset($checked['sendType']['1']); ?> />발송완료</label>
                </td>
            </tr>
            <tr>
                <th>기간검색</th>
