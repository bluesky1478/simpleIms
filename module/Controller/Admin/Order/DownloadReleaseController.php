...
class DownloadReleaseController extends \Controller\Admin\Controller{
    ...
    public function index(){
        $list = $this->getList( '1000000601' , '1000000602', '1000000604', '1000000603');//OEK 25 하계
        $div = "정직원";
        $refineList = [];
    ...
    public function makeExcel($refineList, $div){
        $goodsTitle1 = 'OTIS_OEK 춘추점퍼(26)';
        $goodsTitle2 = 'OTIS_OEK 춘추조끼(26)';
        $goodsTitle3 = 'OTIS_OEK 춘추 긴팔 티셔츠(26)';
        $goodsTitle4 = '하계 바지(26)';

        $totalCnt = 20;
        $addCnt = 2;
        $dpCnt = $totalCnt + $addCnt;

        $fileTitle = "OEK춘추출고패킹리스트_".date('Y-m-d');

        $optionList = DownloadReleaseController::OPTION_LIST;
        ...
        $htmlList[] = ExcelCsvUtil::wrapTh($goodsTitle3,'title','background-color:#fff2cc; font-weight:bold','colspan=8');
        $htmlList[] = ExcelCsvUtil::wrapTh($goodsTitle4,'title','background-color:#fff2cc; font-weight:bold','colspan=7');
    ...
    public function setExcelEach($val, $key, &$htmlList){
        $htmlList[] = '<tr>';
        $optionList = DownloadReleaseController::PACKING_INFO;
    ...
    public function getList($teeCode1,$teeCode2,$teeCode3,$pantsCode){
