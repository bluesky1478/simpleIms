<script type="text/javascript">
    ...
    <?php if('y' === $isReload ) { ?>
        if( $.isEmpty($.cookie('viewTabMode')) ){
            ...
            styleTabMode = '<?=empty($requestParam['styleTabMode']) || 'undefined' == $requestParam['styleTabMode'] ?'basic':$requestParam['styleTabMode']?>';
        }else{
    ...
    <?php } else { ?>
        ...
        styleTabMode = '<?=empty($requestParam['styleTabMode']) || 'undefined' == $requestParam['styleTabMode'] ?'basic':$requestParam['styleTabMode']?>';
    <?php } ?>
    //console.log('styleTabMode:', styleTabMode);
        ...
        ImsService.getData(DATA_MAP.PROJECT,sno).then((data)=>{
            ...
            const initParams = {
                ...
                methods : {
                    ...
                    saveTest : ()=>{
                        console.log('저장 테스트', vueApp.project);
                        $.imsPost('save2', {'target' : 'project', 'saveData' : vueApp.project}).then((data)=>{
                            console.log(data);
                        });
                    },
