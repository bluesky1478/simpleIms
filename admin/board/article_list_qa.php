...
<form name="frmSearch" id="frmSearch" action="article_list.php" class="frmSearch js-form-enter-submit">
    ...
    <div class="search-detail-box">
        <table class="table table-cols">
            ...
            <tr>
                <th >게시판</th>
                <td class=""<?php if ($isShow == 'n') echo 'colspan="3"'; ?> >
            ...
            <tr>
                ...
                <td>
                    <div class="form-inline">
                        ...
                        <input name="searchWord" value="<?= gd_isset($req['searchWord']) ?>" class="form-control width-3xl">
                    </div>
                </td>
                <th >처리상태</th>
                <td>
                    <div class="form-inline">
                        <select name="claimStatus" class="form-control">
                            <option value="">=전체=</option>
                            <?php foreach (\SlComponent\Util\SlCodeMap::NEW_CLAIM_STATUS as $key => $val) { ?>
                                <option value="<?= $key ?>" <?php if ( isset($req['claimStatus'])  && '' !== $req['claimStatus'] && ((int)$req['claimStatus']) === $key) echo 'selected' ?>><?= $val ?></option>
                            <?php } ?>
                        </select>
                    </div>
                </td>
        ...
        if (gd_array_is_empty($bdList['list']) === false) {
            foreach ($bdList['list'] as $val) {
                ...
                <tr class="center">
                    ...
                    <td>
                        <?php if( isset($val['claimStatus']) ) { ?>

                            <?php if(2==$val['claimStatus']) { ?>
                                <span style="color:#1c7430">
                            <?php }else if(9==$val['claimStatus']) { ?>
                                <span class="text-danger">
                            <?php }else{ ?>
                                <span class="text-muted">
                            <?php } ?>
                            <?= \SlComponent\Util\SlCodeMap::NEW_CLAIM_STATUS[$val['claimStatus']]?>
                            </span>

                            <?php if( $val['claimStatus'] > 0 ) { ?>
                                <div ><a href="../order/claim_list.php?key=a.bdSno&keyword=<?=$val['sno']?>&searchDate[]=2000-01-01&searchDate[]=2100-12-31" class="text-blue" target="_blank">클레임처리 열기</a></div>
                            <?php } ?>

                            <?php if( 2 == $val['claimStatus'] ) { ?>
                                <div><small class="text-muted"><?= $val['claimCompleteDt'] ?></small></div>
                            <?php } ?>
                        <?php }else{ ?>
                            <small class="text-muted">일반문의</small>
                        <?php } ?>
                    </td>
                    ...
                    <td>
                        ...
                        <a href="javascript:btnView('<?= $bdList['cfg']['bdId'] ?>',<?= $val['sno'] ?>);" class="btn btn-white btn-sm">열기</a>
                        <?php }?>
