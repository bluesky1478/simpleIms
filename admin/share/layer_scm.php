...
<div class="pop-scm-all-click">
    <input type="checkbox" id="allCheck" value="y" class="js-checkall" data-target-name="layer_scm[]" />
    &nbsp;전체선택
</div>

<ul class="pop-scm-list">
    <?php
    if (gd_isset($data) && is_array($data)) {
        ...
        foreach ($data as $key => $val) {
            if ($val['scmType'] == 'y') {
                ...
                // 5년 경과 주문건의 경우 예외고객사에 탈퇴고객사 포함시켜야함.
                if($mode == 'simple'){
                    $addDisabled = '';
                }
            }
        ...
        $('.js-close').click(function(e){
            ...
            if (applyGoodsCnt > 0) {
                ...
                if (applyGoodsCnt != chkGoodsCnt) {
                    alert('선택한 '+chkGoodsCnt+'개의 고객사 중 '+applyGoodsCnt+'개의 고객사가 추가 되었습니다.');
                }
                ...
                alert('동일한 고객사가 이미 존재합니다.');
            }

            try{
                afterProcScmLayerClose();
            }catch (e){}
    ...
    function displayTemplate(data) {
        ...
        $.each(data.info, function (key, val) {
            ...
            if(data.mode == 'radio' || data.mode == 'layer_radio') {
                ...
                // 고객사 일정 수수료 관리 레이어 선택 시 전체삭제 버튼 앞으로 데이터 삽입
        ...
        if (data.info.length > 0 && !$('#' + data.parentFormID).children().is('h5') && data.mode != 'check') {
            $('#' + data.parentFormID).prepend('<h5>선택된 고객사 : </h5>');
        }
    }
    //-->
</script>
<script type="text/html" id="scmSimpleTemplate">
    <div id="<%=dataFormID%>_<%=scmNo%>" class="btn-group btn-group-xs">
        <input type="hidden" name="<%=dataInputNm%><%=inputArr%>" value="<%=scmNo%>" class="selected-scm-no">
...
<script type="text/html" id="scmSearchTemplate">
    <div id="<%=dataFormID%>_<%=scmNo%>" class="btn-group btn-group-xs">
        <input type="hidden" name="<%=dataInputNm%>[]" value="<%=scmNo%>" class="selected-scm-no">
