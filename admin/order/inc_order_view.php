...
$hkCodeList = DBUtil2::getList('sl_3plProduct', 'scmNo', 6);
foreach($hkCodeList as $hkCode){
    $hkCodeMap[$hkCode['thirdPartyProductCode']] = $hkCode['productName'].' '.$hkCode['optionName'];
}
    ...
    <table class="table table-rows">
        ...
        if (count($orderGridConfigList) > 0) {
            foreach($orderGridConfigList as $gridKey => $gridName){
                ...
                else if ($gridKey === 'processStatus' && empty($isProvider) ){
                    echo "<th>".$gridName." <input type='button' value='로그' class='btn btn-sm btn-white js-order-log' /></th>";
                }
                else if ($gridKey === 'goodsPrice'){
                    echo "<th>".$gridName."</th>";
                    echo "<th>무상제공금액</th>";
                    echo "<th>본사지불금액(할인)</th>";
                    echo "<th>구매자지불금액</th>";
                    echo "<th>적용정책</th>";
                }
                else if ($gridKey === 'goodsCnt'){
                    if ('o' == substr($data['orderStatus'], 0, 1)) {
                        echo "<th>".$gridName.'<div><input type="button" value="일괄수정" class="btn btn-sm btn-red js-modify-cnt" style="margin-top:3px;width:67px"/></div>'."</th>";
                    }else{
                        echo "<th>수량</th>";
                    }
                }
                            ...
                            ?>
                                ...
                                <?php if($gridKey === 'orderGoodsNo'){ // 상품주문번호 ?>
                                    <td>
                                        ...
                                        <?php if( !empty($val['handleMode']) && $val['handleMode'] === 'z'){ ?>
                                            <div class="delivery-change-area">


                                                <?php if(empty($isProvider)){ ?>

                                                <label class="radio-inline">
                                                    <input type="radio" class="deliveryCollectPre" name="goodsDeliveryCollectFl[<?=$val['sno']?>]" value="pre" <?= 'later'!==$val['goodsDeliveryCollectFl']?'checked':''; ?>/>선불
                                                </label>
                                                <label class="radio-inline">
                                                    <input type="radio" class="deliveryCollectLater" name="goodsDeliveryCollectFl[<?=$val['sno']?>]" value="later" <?= 'later'===$val['goodsDeliveryCollectFl']?'checked':''; ?>/>착불
                                                </label>

                                                <div>
                                                    <div class="btn btn-white btn-set-handle-delivery" data-sno="<?= $val['sno']; ?>">저장</div>
                                                </div>
                                                <?php } ?>
                                        ...
                                        <div class="text-muted font-11">Op:<?=$val['optionSno']?></div>

                                        <div class="text-muted font-11">
                                            아시아나 정보
                                            <br>:<?=$val['asianaOrderInfo']['companyId']?> <?=$val['asianaOrderInfo']['name']?>
                                        </div>
                                ...
                                <?php if($gridKey === 'orderGoodsNm'){ // 주문상품 ?>
                                    <td class="text-left">
                                        ...
                                        <?php if( 20 == $val['scmNo'] && empty($val['optionInfo']) !== false ) { ?>
                                            <?=$val['goodsNmStandard']?>
                                        <?php } ?>
                                        ...
                                        <div class="info">
                                            ...
                                            */
                                            if (20 != $val['scmNo']) {
                                                if( !empty($val['optionInfo'][0]['optionCode']) ){
                                                    echo '<div class="font-kor" title="상품코드">[' . $val['optionInfo'][0]['optionCode'] . ' ' . $hkCodeMap[$val['optionInfo'][0]['optionCode']] . ']</div>';
                                                }else{
                                                    echo '<div class="font-kor" title="상품코드">[' . $val['optionInfo'][1]['optionCode'] . ' ' . $hkCodeMap[$val['optionInfo'][1]['optionCode']] . ']</div>';
                                                }
                                ...
                                <?php if($gridKey === 'goodsCnt'){ // 수량 ?>
                                    <td class="text-center">
                                        <?php if('o' == substr($val['orderStatus'],0,1)){ ?>
                                            <div class="form-inline" >
                                                <input type="text" class="form-control goods-cnt inp-goods-cnt" name="goodsCnt[<?= $val['sno']; ?>]" value="<?=$val['goodsCnt']?>" data-sno="<?=$val['sno']?>"  style="width:50px"  /> 개
                                            </div>
                                        <?php }else{ ?>
                                ...
                                <?php if($gridKey === 'goodsPrice'){ // 상품금액 ?>
                                    ...
                                    <td class="text-right">
                                        <div><?= gd_currency_display($val['applyPolicy']['freeDcAmount'])?></div>
                                        <div>
                                            <small><?= $val['applyPolicy']['freeDcCount']>0? '(무상제공 : '.$val['applyPolicy']['freeDcCount'].'개)':''?></small>
                                        </div>
                                    </td>
                                    <td class="text-right">
                                        <div><?= gd_currency_display($val['applyPolicy']['companyPayment'])?></div>
                                    </td>
                                    <td class="text-right">
                                        <div><?= gd_currency_display($val['applyPolicy']['buyerPayment'])?></div>
                                    </td>
                                    <td class="text-right">
                                        <?php if( !empty($val['applyPolicy']['policyInfo']['freeDcCount']) ) { ?>
                                            <div class="apply-policy">무상제공갯수 : <?=$val['applyPolicy']['policyInfo']['freeDcCount']?>개</div>
                                        <?php } ?>
                                        <?php if( !empty($val['applyPolicy']['policyInfo']['policySaleName']) ) { ?>
                                            <div class="apply-policy">지불비율 : (본)<?=$val['applyPolicy']['policyInfo']['companyRatio']?>% / (구) <?=$val['applyPolicy']['policyInfo']['buyerRatio']?>%</div>
                                        <?php } ?>
                                    </td>
                                <?php } // 상품금액 ?>
...
<?php if (isset($data['goods']) === true) { ?>
    <div class="table-action"  <?php if( $isProvider ) { ?>style="display: none"<?php } ?> >
...
<script type="text/javascript">
    $(function(){
       $('.btn-set-handle-delivery').click(function(){
            let sno = $(this).data('sno');
            let deliveryValue = $(this).closest('.delivery-change-area').find('.deliveryCollectPre').is(':checked') ? 'pre':'later';
            let params = {
                mode:'setExchangeDelivery',
                sno:sno,
                deliveryValue:deliveryValue
            }
           $.post("claim_ps.php", params, function (result) {
                location.reload();
           });
       });
    });
...
//                    layer_popup(data, dom.statusSelect.find('option:selected').text(), 'wide');
        ...
        //일괄 수정
        $('.js-modify-cnt').click(function(){
            var param = [];
            $('.inp-goods-cnt').each(function(){
                var sno = $(this).data('sno');
                param.push(
                    {
                        sno : sno
