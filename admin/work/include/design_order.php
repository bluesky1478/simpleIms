            ...
            <tr>
                ...
                <td>
                    <div class="form-inline">
                        ...
                        <div class="text-danger" v-show="!$.isEmpty(items.docData.sendDt)"><b>{% items.docData.sendDt %} 에 발송하였습니다.</b></div>
                    </div>
                </td>
            </tr>
            <tr>
...
<div v-for="(item, index) in items.docData.sampleData" :key="index" v-show="index == items.showSelectedStyle" >
        ...
        <div>
            <table class="table table-cols">
                ...
                <tr>
                    ...
                    <td>
                        <label class="radio-inline" >
                            <input type="radio" :name="'sizeDisplay' + index" v-model="item.sizeDisplay" value="숫자 - 100" />숫자표기
                            <label class="radio-inline" >
                            </label>
                            <input type="radio" :name="'sizeDisplay' + index" v-model="item.sizeDisplay" value="영문 - L" />영문표기
                        </label>
                    </td>
                    <th>핏</th>
                    <td>
    ...
    <div class="col-xs-12">
        <div class="table-title ">
            <div class="flo-left" >상품 썸네일/도식화 이미지 미리보기</div>
            <div class="flo-right" ></div>
        </div>
        <table class="table table-cols">
            <tr>
        ...
        <div  style="clear:both">
            <table class="table table-cols center-header center-contents sample-spec-table" >
                ...
                <tr >
                    ...
                    <td v-for="(optionItem, optionIndex) in item.optionList" class="text-center"><!-- Loop는 옵션의 수만큼 -->
                        {% $.setNumberFormat(optionItem.optionTotalCount) %}개 / {% $.setNumberFormat(optionItem.optionInputTotalCount) %}개
                    </td>
                    <td>&nbsp</td>
                </tr>
            </table>
...
<div class="col-xs-12"  v-show="items.version > 0">
    ...
    <div>
        ...
        <div class="form-inline" style="padding:5px">
            <textarea class="form-control comment-text-area" v-model="comment" style="width:85%; height:60px"></textarea>
...
<script type="text/javascript">
    ...
    let externSelectDocument = function(itemDocData, docData, parentApp){
        ...
        docData.deliveryManager = srcDocData.companyManager;
        docData.deliveryPhone = srcDocData.companyManagerPhone;

        docData['sampleData'] = [];
           let workDataList = $.copyObject(itemDocData.docData.recentWorkData);
        workDataList.forEach((workData)=>{
            //option
            workData.optionList.forEach((optionData)=>{
                optionData.optionInputTotalCount = 0;
            });
            //type
            workData.typeList.forEach((typeData)=>{
                typeData.typeInputTotalCount = 0;
                if( (typeof typeData.inputCount) == 'undefined' ){
                    typeData.inputCount = [];
                }
                workData.optionList.forEach(()=>{
                    typeData.inputCount.push('');
                });
            });
            workData.itemInputTotalCount = 0;
            docData['sampleData'].push(workData);
        });
    }
    ...
    vueUpdatedCallBackFnc = function(el){
        for(let key in el.items.docData.sampleData){
            ...
            if( typeof thumbnailDropzone.fileList[key] == 'undefined' ){
                //console.log(' SET thumbnail Drop zone ');
                let completeProc = function(dropzoneObject, file){
                    let promiseInitData = WorkDocument.getDocument(DOC_DEPT,DOC_TYPE, documentSno);
                    promiseInitData.then((jsonResult)=>{
                        let fieldName1 = dropzoneObject.optionList[key].fieldName1;
                        let fieldName2 = dropzoneObject.optionList[key].fieldName2;
                        el.items.docData.sampleData[key]['fileThumbnail'] = jsonResult.data.docData[fieldName1][key][fieldName2];
                        dropzoneObject.fileList[key].removeFile(file);
                    });
                }
                ...
                let dropzoneSetParams = {
                    workUrl : 'work_ps.php?mode=uploadDocumentFileDropzone',
                    dropzoneId : "div#thumbnail-dropzone" + key,
                    fieldName1 : 'sampleData',
                    fieldName2 : 'fileThumbnail',
                };
                thumbnailDropzone.setDropzone( el.items.docData.sampleData[key].fileThumbnail ,key, dropzoneSetParams);
                thumbnailDropzone.fileList[key].on("complete", function(file) {
                    completeProc(thumbnailDropzone, file);
                });
                thumbnailDropzone.fileList[key].on("addedfile", function(file) {
                    let imageUrl = URL.createObjectURL(file);
                    el.items.docData.sampleData[key].fileThumbnailPreview.push({
                        uuid : file.upload.uuid,
                        imageUrl : imageUrl,
                    });
                });
                thumbnailDropzone.fileList[key].on("removedfile", function(file) {
                    for(let idx in el.items.docData.sampleData[key].fileThumbnailPreview){
                        if( file.upload.uuid == el.items.docData.sampleData[key].fileThumbnailPreview[idx].uuid){
                            el.items.docData.sampleData[key].fileThumbnailPreview.splice(idx,1);
                        }
                    }
                });
                //---
