    ...
    <div class="">
        <div>
            <table class="table table-cols table-pd-5" style="border-top:none !important;">
                ...
                <tr>
                    <th>비축 자재명</th>
                    <td><?=$fabricOutputInfo['fabricName']?></td>
                    <th>현재수량</th>
                    <td>{% $.setNumberFormat(limitQty) %}</td>
                </tr>
                <tr>
                    <th>혼용율</th>
                    <td><?=$fabricOutputInfo['fabricMix']?></td>
                    <th>출고수량</th>
                    <td>
                ...
                <tr>
                    <th>색상</th>
                    <td><?=$fabricOutputInfo['color']?></td>
                    <th></th>
                    <td></td>
                </tr>
                <tr>
                    <th>소유권</th>
                    <td><?=$fabricOutputInfo['inputOwn']?></td>
                    <th></th>
                    <td></td>
                </tr>
                <tr>
...
<script type="text/javascript">
    ...
    $(()=>{
        ...
        ImsBoneService.setData(serviceData,{
            isModify : true,
            limitQty : <?=$fabricOutputInfo['remainQty']?>,
            sendData : {
                'sno' : '<?=$fabricOutputInfo['sno']?>',
                'outQty' : '<?=number_format($fabricOutputInfo['outQty'])?>',
                'outReason' :'<?=addslashes($fabricOutputInfo['outReason'])?>',
            }
        });
        ...
        ImsBoneService.setMethod(serviceData,{
            save : ()=>{
                ...
                $.imsPost('modifyStoredFabricOutput', vueApp.sendData).then((data)=>{
                    $.imsPostAfter(data,(data)=>{
                        $.msg('출고 수정 완료','','success').then(()=>{
                            parent.opener.parent.opener.location.reload(); //부모부모창 갱신.
