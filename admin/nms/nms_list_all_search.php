...
<div class="search-detail-box form-inline">
    <table class="table table-cols table-td-height0 table-pd-5 table-pdl-7 border-top-none ">
        ...
        <tr>
            ...
            <td colspan="3">
                <div :class="'btn btn-white '+(searchCondition.iSchManagerSno==<?=\Session::get('manager.sno')?>?'btn-blue-line':'')" @click="searchCondition.iSchManagerSno=<?=\Session::get('manager.sno')?>; refreshList(1);">담당 프로젝트 검색</div>
                <div class="dp-flex">
                    담당영업 :
                    <div class="btn btn-white btn-sm" @click="searchCondition.iSchManagerSno=-1; refreshList(1);">전체</div>
                    <?php foreach($salesManagerList as $salesSno => $sales) { ?>
                        <div :class="'btn btn-white btn-sm '+(searchCondition.iSchManagerSno==<?=$salesSno?>?'btn-blue-line':'')" @click="searchCondition.iSchManagerSno=<?=$salesSno?>; refreshList(1);"><?=$sales?></div>
                    <?php } ?>
                </div>
                <div class="dp-flex mgt5">
                    디자이너 :
                    <div class="btn btn-white btn-sm" @click="searchCondition.iSchManagerSno=-1;">전체</div>
                    <?php foreach($designManagerList as $desingerSno => $designer) { ?>
                        <div :class="'btn btn-white btn-sm '+(searchCondition.iSchManagerSno==<?=$desingerSno?>?'btn-blue-line':'')" @click="searchCondition.iSchManagerSno=<?=$desingerSno?>; refreshList(1);"><?=$designer?></div>
                    <?php } ?>
                </div>
            </td>
        </tr>
