<div class="page-header js-affix">
    <h3><?= end($naviMenu->location); ?>(지급/차감) </h3>
    <div class="btn-group">
        <input type="button" value="목록" class="btn btn-white btn-icon-list" onclick="goList('./point_list.php');" />
        <input type="button" value="포인트 지급/차감" class="btn btn-red btn-register"/>
    </div>
</div>
            ...
            <tr>
                ...
                <td>
                    ...
                    <div class="contents">
                        ...
                        <div>- 엑셀 항목 중 “아이디”와 “지급/차감 포인트”는 필수값입니다.</div>
                        <div>- [샘플파일보기] 버튼을 눌러 다운로드 되는 엑셀 샘플파일을 참고하시기 바랍니다.</div>
                        <div>- 차감 처리 시 "지급/차감 포인트"에 마이너스 숫자(예시) -400)을 입력하시면 되고, 차감할 포인트보다 보유 포인트가 적은 회원은 마이너스 처리됩니다.</div>
                    </div>
                </td>
            </tr>
            </tbody>
    ...
    $(document).ready(function () {
        ...
        $formSetup.validate({
            ...
            rules: {
                ...
                reasonCd: "포인트를 지급/차감한 사유를 입력해주세요.",
                mileageValue: "지급/차감할 포인트 금액을 입력해주세요.",
                "batchTarget[]": "포인트를 지급/차감할 대상 회원을 확인해주세요."
                ...
                ajax_with_layer('member_batch_ps.php', params, function (data, textStatus, jqXHR) {
                    layer_close();
                    if (data[0]) {
                        ...
                        if (resultStorage && (resultStorage.excludeCount > 0)) {
                            if (resultStorage.excludeCount == resultStorage.totalCount) {
                                dialogMessage = "선택한 회원의 포인트가 차감할 금액보다 부족합니다. 대상회원을 다시 선택해주세요.";
                            }
                        }

                        BootstrapDialog.confirm({
                            title: "포인트 지급/차감 처리 완료",
                            message: dialogMessage,
                            btnOKLabel: "처리내역 확인",
                            btnCancelLabel: "지급/차감 계속진행",
                            callback: function (result) {
                                if (result) {
                                    top.location.href = 'point_list.php';
                                } else {
                        ...
                        alert("포인트 지급/차감 처리 중 오류가 발생하였습니다.");
                    }
                });
            }
        });
        ...
        $('#formSearch').validate({
            submitHandler: function (form) {
                ...
                window.location.href = 'point_add.php?' + queryString1 + '&' + queryString2;
            }
        });
