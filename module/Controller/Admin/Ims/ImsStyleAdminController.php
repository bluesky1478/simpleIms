 ...
 * 문서 리스트
 */
class ImsStyleAdminController extends \Controller\Admin\Controller{
    ...
    public function index(){
        ...
        $tableBodyData = str_replace('**eworkMaterialFl**','<i aria-hidden="true" class="fa fa-lg fa-check-circle text-green cursor-pointer" v-if="each.eworkMaterialFl >= 1" @click="openCommonPopup(\'ework\', 1300, 850, {sno:each.sno, tabMode:\'material\'})"></i>',$tableBodyData);

        $tableBodyData = str_replace('**eworkPackingFl**',
            '<i aria-hidden="true" class="fa fa-lg fa-check-circle text-green cursor-pointer" v-if="1==each.eworkPackingFl" @click="openCommonPopup(\'ework\', 1300, 850, {sno:each.sno, tabMode:\'packing\'})"></i>'.
            '<i aria-hidden="true" class="fa fa-lg fa-stop-circle text-orange cursor-pointer" v-if="2==each.eworkPackingFl" @click="openCommonPopup(\'ework\', 1300, 850, {sno:each.sno, tabMode:\'packing\'})"></i>'
            ,$tableBodyData);

        $tableBodyData = str_replace('**eworkMarkFl**',
            '<i aria-hidden="true" class="fa fa-lg fa-check-circle text-green cursor-pointer" v-if="1==each.eworkMarkFl" @click="openCommonPopup(\'ework\', 1300, 850, {sno:each.sno, tabMode:\'mark\'})"></i>'.
            '<i aria-hidden="true" class="fa fa-lg fa-stop-circle text-orange cursor-pointer" v-if="2==each.eworkMarkFl" @click="openCommonPopup(\'ework\', 1300, 850, {sno:each.sno, tabMode:\'mark\'})"></i>'
            ,$tableBodyData);

        $tableBodyData = str_replace('**eworkBatekFl**',
            '<i aria-hidden="true" class="fa fa-lg fa-check-circle text-green cursor-pointer" v-if="1==each.eworkBatekFl" @click="openCommonPopup(\'ework\', 1300, 850, {sno:each.sno, tabMode:\'batek\'})"></i>'.
            '<i aria-hidden="true" class="fa fa-lg fa-stop-circle text-orange cursor-pointer" v-if="2==each.eworkBatekFl" @click="openCommonPopup(\'ework\', 1300, 850, {sno:each.sno, tabMode:\'batek\'})"></i>'
            ,$tableBodyData);
     ...
     * @param $dpData
     */
    public function simpleExcelDownload5($request, $dpData){
        ...
        $dpData = [
            ...
            'reqFactoryNm' => ['name'=>'생산처','col'=>'10'],
            //Fabric Join .
            'estimateCount' => ['name'=>'견적수량'],
            'salePrice' => ['name'=>'판매가'],
            'contents_totalCost' => ['name'=>'생산가'],
            'contents_laborCost' => ['name'=>'공임'],
            'contents_exchange' => ['name'=>'환율'],
            'contents_exchangeDt' => ['name'=>'환율기준일'],
            'contents_exchange2' => ['name'=>'달러변환'],
            'contents_marginCost' => ['name'=>'마진'],
            'contents_dutyCost' => ['name'=>'물류관세'],
            'contents_managementCost' => ['name'=>'관리비'],
            'contents_deliveryType' => ['name'=>'운송형태'],
        ];
        ...
        foreach($list['list'] as $key => $val){
            ...
            foreach($dpData as $dpKey => $dpValue){
                if(true !== $dpValue['skip']){
                    $splitKey = explode('_',$dpKey);
                    if( 'contents' !== $splitKey[0] ){
                        $contentsRows[] = ExcelCsvUtil::wrapTd($val[$dpKey]);
                    }else{
                        $valueKey = $splitKey[1];
                        $decodeContents = json_decode($val[$splitKey[0]], true);
                        if('exchange2' == $valueKey){
                            if( $decodeContents['laborCost'] && $decodeContents['exchange'] ){
                                $exchange = round($decodeContents['laborCost'] / $decodeContents['exchange'],2);
                                $contentsRows[] = ExcelCsvUtil::wrapTd($exchange);
                            }else{
                                $contentsRows[] = ExcelCsvUtil::wrapTd('0');
                            }
                        }else{
                            $contentsRows[] = ExcelCsvUtil::wrapTd($decodeContents[$valueKey]);
                        }
                    }
                }
            }
        ...
        $simpleExcelComponent = SlLoader::cLoad('Excel','SimpleExcelComponent','sl');
        $simpleExcelComponent->simpleDownload('스타일_생산가격', $title, implode('',$contents));
    }
