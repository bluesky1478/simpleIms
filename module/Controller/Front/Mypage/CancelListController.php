    ...
    public function index()
    {
        try {
            ...
            if ($orderBasic['userHandleFl'] === 'y' && (empty($mode) === true || $mode == 'cancelRequest')) {
                $mode = 'cancelRequest';
                $requestDataParam['searchDate'][] = $startDate;
                $requestDataParam['searchDate'][] = $endDate;
                $requestDataParam['key'] = 'memId';
                $requestDataParam['keyword'] = \Session::get('member.memId');
                $service = SlLoader::cLoad('claim','claimService');;
                $claimList = $service->getClaimList($requestDataParam);
                $orderData = $claimList['data'];
                $this->getView()->setPageName('mypage/cancel_list_sl');
                $this->setData('claimStatusMap', SlCodeMap::CLAIM_STATUS); //Claim Type Code
                $this->setData('claimTypeMap', SlCodeMap::CLAIM_TYPE); //Claim Type Code
                $this->setData('reqGoodsCnt',$claimList['reqGoodsCnt']);
            } else {
                $mode = 'cancel';
