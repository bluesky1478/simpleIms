    ...
    <table class="table table-cols  xsmall-picker " >
        ...
        <tr >
            <td >
                ...
                <form id="fileCare" class="set-dropzone mgt5" @submit.prevent="uploadFiles" >
                    <div class="fallback">
                        <input name="upfile" type="file" multiple @change="file.files = $event.target.files" />
                    </div>
                </form>
                <div class="btn btn-sm btn-gray mgt15" @click="delFile('fileCare')">- 파일삭제</div>
            </td>
            <td >
                <simple-file-list :files="mainData.ework.fileList.filePosition"></simple-file-list>
                <form id="filePosition" class="set-dropzone mgt5" @submit.prevent="uploadFiles" >
                    <div class="fallback">
                        <input name="upfile" type="file" multiple @change="file.files = $event.target.files" />
                    </div>
                </form>
                <div class="btn btn-sm btn-gray mgt15" @click="delFile('filePosition')">- 파일삭제</div>
            </td>
        </tr>
        <tr>
            <td >
        ...
        <tr>
            <td colspan="99" class="text-center">
                <textarea class="form-control" rows="5" v-model="mainData.ework.data.warnPosition" @keyup="eworkUpdate('warnPosition',mainData.ework.data.warnPosition)" @blur="eworkUpdate('warnPosition',mainData.ework.data.warnPosition)"></textarea>
            </td>
        </tr>
        </tbody>
    </table>
