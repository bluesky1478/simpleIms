...
<form id="frmList" name="frmList" action="" method="post">
        ...
        if (gd_isset($data)) {
            ...
            foreach ($data as $key => $val) {
                ...
                <tr class="text-center">
                    ...
                    <td class="text-left">
                        <?php echo gd_select_box(null, 'scmList['.$val['sno'].']', $scmList, '', $val['scmNo'], '=== 연결안됨 ===' ); ?>
                    </td>
                    <td>
    ...
    <div class="table-action">
        ...
        <input type="button" value="선택 연결" class="btn btn-red js-link_selected mgl10" />
    </div>
</form>
...
<script type="text/javascript">
    ...
    // 선택한 팝업 연결 저장
    $('.js-link_selected').click(function () {

        BootstrapDialog.show({
            title: '선택한 팝업 고객사 연결',
            message: '팝업을 정말로 연결 하시겠습니까?',
            buttons: [
                {
                    id: 'btn-cancel',
                    label: '연결 취소',
                ...
                },
                {
                    ...
                    action: function(dialog) {
                        ...
                        $delButton.spin();
                        dialog.setClosable(false);
                        dialog.setMessage('팝업을 연결 중입니다.');

                        $('#frmList input[name=\'mode\']').val('link_selected');
                        $('#frmList').attr('method', 'post');
                        $('#frmList').attr('target', 'ifrmProcess');
                        $('#frmList').attr('action', '../scm/scm_custom_ps.php');
                        $('#frmList').submit();
                    }
                }]
        });
