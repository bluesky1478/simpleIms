<div class="table-title gd-help-manual">
    기본정보
    [<a href="<?=URI_HOME?>/member/auto_login.php?loginMemId=<?= $data['memId'] ?>" target="_blank">임시로그인</a>]
</div>
<div class="form-inline">
    <table class="table table-cols">
        ...
        <tr>
            <th>TKE 회원유형 및<br>구매제한</th>
            <td >
                <?=gd_select_box('memberType', 'memberType', \SlComponent\Util\SlCodeMap::MEMBER_TYPE, null, $memberType, '==없음==', 'form-control js-member-type-change width-lg inline '); ?>
                <input type="text" id="buyLimitCount" name="buyLimitCount" value="<?=$buyLimitCount?>" class="form-control "  style="width:50px; text-align: right" > 개 제한
            </td>
            <th>구매한계 수량</th>
            <td >
                <input type="text" id="buyLimitCount" name="ex4" value="<?=$data['ex4']?>" class="form-control "  style="width:50px; text-align: right" > 개 
            </td>
        </tr>
        <tr>
            <th>한국타이어<br> 매장유형</th>
            <td colspan="3">
                ...
                <input type="checkbox" name="hankookTypeCheck" value="<?=$hankookKey?>"  class="chk-hankook-type" <?=$hankookKey & $data['hankookType'] ? 'checked' : ''  ?> >
                <?=$hankookValue?>
                <?php } ?>
                <input type="hidden" id="hankookType" name="hankookType" value="<?=$data['hankookType']?>">
            </td>
        </tr>
    </table>
</div>
