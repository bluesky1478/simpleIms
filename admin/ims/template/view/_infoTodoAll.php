        ...
        <div class="col-xs-6 new-style">
            <div class="table-title gd-help-manual">
                <div class="flo-left area-title">
                    ...
                    <?=$todoInfo['title']?> <span class="font-13">- {% todoListAll.<?=$todoInfo['dept']?>.list.length %}가지 할일이 있습니다.
                </div>
                <div class="flo-right">

                    <span class="radio-inline">
                        <label class="radio-inline noto font-14">
                            <input type="radio" name="myTodo<?=$todoInfo['dept']?>"  value="list" class="mgt5" v-model="todoListAll.<?=$todoInfo['dept']?>.status" /> 할일 목록
                        </label>
                        <label class="radio-inline noto font-14" >
                            <input type="radio" name="myTodo<?=$todoInfo['dept']?>"  value="completeList" class="mgt5" v-model="todoListAll.<?=$todoInfo['dept']?>.status" /> 완료건
                        </label>
            ...
            <table class="table table-cols table-default-center mgt5 table-td-height0" id="main-table">
                ...
                <thead>
                <tr v-show="'list' === todoListAll.<?=$todoInfo['dept']?>.status">
                    <th>번호</th>
                    <th>요청번호</th>
                    <th>요청자</th>
                    <th>대상</th>
                ...
                <tbody v-show="todoListAll.<?=$todoInfo['dept']?>.<?=$listType?>.length > 0 && '<?=$listType?>' === todoListAll.<?=$todoInfo['dept']?>.status ">
                <tr v-for="(todoData, todoIndex) in todoListAll.<?=$todoInfo['dept']?>.<?=$listType?>" v-show="todoListAll.<?=$todoInfo['dept']?>.maxRow > todoIndex">
                    <td >{% todoListAll.<?=$todoInfo['dept']?>.<?=$listType?>.length - todoIndex %}</td>
                    <td >{% todoData.sno %}</td>
                    <td >
                ...
                <tr v-show="todoListAll.<?=$todoInfo['dept']?>.<?=$listType?>.length > todoListAll.<?=$todoInfo['dept']?>.maxRow" >
                    <td colspan="99" style="border-bottom: none">
                        <div class="btn btn-sm btn-gray-line cursor-pointer hover" @click="todoListAll.<?=$todoInfo['dept']?>.maxRow += 5">+more</div>
                    </td>
                </tr>
                </tbody>
                <tbody v-show="0 >= todoListAll.<?=$todoInfo['dept']?>.<?=$listType?>.length && '<?=$listType?>' === todoListAll.<?=$todoInfo['dept']?>.status ">
                <tr>
