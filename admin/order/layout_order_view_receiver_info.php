...
<table class="table table-cols">
    ...
    <tr>
        ...
        <td>
            ...
            <?php if (empty($data['receiverCellPhone'][1]) === false && empty($data['isDefaultMall']) === false && !$isProvider ) { ?>
    ...
    <tr>
        ...
        <tr>
            ...
            <td>
                <ul>
                    <li>회원구분 : <?=empty($orderAddedData['memberType'])?'오픈패키지':'재계약매장'?></li>
                    <li>선물금액 : <?=number_format($orderAddedData['giftAmount'])?>원</li>
                    <li>적립금액 : <?=number_format($orderAddedData['addDeposit'])?>원</li>
                    <?php if( '00000-00-00' != $orderAddedData['reqDeliveryDt'] ) { ?>
                        <li><strong style="font-size:15px;">배송요청일 : <span class="text-danger"><?=$orderAddedData['reqDeliveryDt']?></span></strong></li>
                    <?php } ?>
                </ul>
            </td>
        </tr>
