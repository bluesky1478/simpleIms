 ...
 * @author  su
 */
class DownloadReleaseController extends \Controller\Admin\Controller{
    ...
    public function index(){
        $packingInfoList = DownloadTkeReleaseController::OPTION_LIST;

        foreach($packingInfoList as $goodsNo => $packingInfo){
            $goodsOptionList = DBUtil2::getList(DB_GOODS_OPTION, 'goodsNo', $goodsNo);
            //foreach($goodsOptionList)
            //$packingInfoList[$goodsNo]['option'] =
        }

        //gd_debug('출고리스트 뻡기');
        $getValue = \Request::get()->toArray();
        if( !empty($getValue['mode']) && 'partner' === $getValue['mode'] ){
            //$list = $this->getList( '1000000329' , '1000000331' ); //23 하계 파트너
            $list = $this->getList( '1000000342' , '1000000343' ); //23동계 파트너
            $div = "파트너";
        }else{
            $list = $this->getList( '1000000373' , '1000000374' );//24OEK하계
            //$list = $this->getList( '1000000340', '1000000341' ); //24
            //$list = $this->getList( '1000000334' ); //23춘추
            $div = "정직원";
        }
    ...
    public function makeExcel($refineList, $div){
        ...
        $fileTitle = "({$div})출고패킹리스트_".date('Y-m-d');
        ...
        $htmlList[] = '<tr>';
        //$htmlList[] = ExcelCsvUtil::wrapTh($fileTitle,'title','background-color:#ffffff; font-weight:bold; font-size:20px','colspan=24');
        //$htmlList[] = ExcelCsvUtil::wrapTh($fileTitle,'title','background-color:#ffffff; font-weight:bold; font-size:20px','colspan=13');
        ...
        $htmlList[] = ExcelCsvUtil::wrapTh($goodsTitle1,'title','background-color:#d9e1f2; font-weight:bold','colspan=9');
        $htmlList[] = ExcelCsvUtil::wrapTh($goodsTitle2,'title','background-color:#fff2cc; font-weight:bold','colspan=11');
        $htmlList[] = ExcelCsvUtil::wrapTh('셔츠+바지 합계','title','background-color:#fff2cc; font-weight:bold','rowspan=2');
        //$htmlList[] = ExcelCsvUtil::wrapTh('구분','title','background-color:#f0f0f0; font-weight:bold','rowspan=2');

        $htmlList[] = ExcelCsvUtil::wrapTh('ID','title','background-color:#f0f0f0; font-weight:bold','rowspan=2');
        $htmlList[] = ExcelCsvUtil::wrapTh('Nick','title','background-color:#f0f0f0; font-weight:bold','rowspan=2');
        ...
        $htmlList[] = ExcelCsvUtil::wrapTh('셔츠:'.number_format($totalData['tee']).'개','title','background-color:#f0f0f0; font-weight:bold');
    ...
    public function setExcelEach($val, $key, &$htmlList){
        ...
        foreach( $optionList as $option ){
            ...
            $eachTotal += $val['option'][$option];
        }

        //$htmlList[] = ExcelCsvUtil::wrapTd($val['info']['grade'], '', 'text-align:center');
    ...
    public function setEach($each, $key, &$mixData){
        ...
        $mixData[$key]['option'][$optionValue] += $each['goodsCnt'];
    }


    public function getList($teeCode, $pantsCode){
        ...
        $sql = "
            ...
            join es_memberGroup f    
              on e.groupSno = f.sno
            where '2023-12-31 23:59:59' >= a.regDt
            and left(a.orderStatus,1) in ('s','d') 
                  -- a.orderStatus = 'p3' 
            and e.groupSno = 5 -- NI  
            and a.goodsNo in ( 
            '{$teeCode}' 
