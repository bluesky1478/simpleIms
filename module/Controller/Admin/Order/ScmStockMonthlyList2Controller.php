...
class ScmStockMonthlyList2Controller extends \Controller\Admin\Order\ScmStockMonthlyListController{
    public function index(){
        parent::index();
        $isProvider = Manager::isProvider();
        if( $isProvider ){
            $this->callMenu('statistics', 'scm', 'goodsCountMonthly2');
        }else{
            $this->callMenu('order', 'order', 'stock_monthly_list2');
        }
        $this->getView()->setPageName('order/scm_stock_monthly_list2.php');
    }
    ...
    public function simpleExcelDownload($getData){
        ...
        $totalBody .= "<tr><td colspan='{$topTitleColspan}' style='font-size:20px;font-weight: bold;text-align: center; '>월별 입고 현황 {$startDate} ~ {$endDate} </td></tr>";
        $totalBody .= $excelBody;
        $totalBody .= "</table>";

        $simpleExcelComponent->downloadCommon($totalBody, '월별입고현황_'.$startDate.'∽'.$endDate);
