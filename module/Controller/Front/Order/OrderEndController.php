...
class OrderEndController extends \Bundle\Controller\Front\Order\OrderEndController
{
    ...
    public function index()
    {
        try {
            ...
            $deliveryInfo = array(); //추가 부분
            foreach($orderInfo['goods'] as $key => $val){
                //gd_debug($val['deliverySno']);
                //gd_debug($val['deliveryPolicy']);
                $deliveryInfo[$val['deliverySno']] = json_decode($val['deliveryPolicy'],true);
                $categoryList = $goodsAdmin->getGoodsLinkCategory($val['goodsNo']);
                $orderInfo['goods'][$key]['cateCd'] = $categoryList[0]['cateCd'];
                if( $orderService->isPaymentCategory($val['cateCd']) ){
                    $isPaymentOrder = true;
                }
            }

            if($isPaymentOrder){
                $this->getView()->setPageName('order/order_payment_end');
            }


            $this->setData('naverCommonInflowScript', gd_isset($naverCommonInflowScript));
            $this->setData('orderInfo', gd_isset($orderInfo));
            $this->setData('deliveryInfo', gd_isset($deliveryInfo));
            ...
            if(34==$scmNo){
                $this->getView()->setPageName('order/order_end_asiana');
            }
