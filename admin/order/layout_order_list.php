                                    ...
                                    foreach($orderGridConfigList as $gridKey => $gridName){
                                        ...
                                        <?php if($gridKey === 'orderNo'){ //주문번호 ?>
                                            <td class="order-no">
                                                ...
                                                <!--출고승인여부 표시-->
                                                <?php if( !empty($val['orderAcctStr']) ) { ?>
                                                    <div>
                                                        <span class="label <?=$val['orderAcctClass']?>"><?=$val['orderAcctStr']?></span>
                                                        <?php if( $val['orderAcctRegDt'] !== $val['orderAcctModDt'] ) { ?>
                                                        <br><small><?=gd_date_format('Y-m-d',$val['orderAcctModDt'])?></small>
                                                        <?php } ?>
                                                    </div>
                                                <?php } ?>

                                                <?php if( !empty($val['userConsultMemo']) ) { ?>
                                                    <br><small class="text-danger">(추가결제 주문서)</small>
                                                <?php } ?>


                                                <?php if( !empty($val['deliveryBoxType']) ) { ?>
                                                    <div style="margin-top:5px ">
                                                        <small style="padding:2px 5px; background-color:#0c4da2; color:#fff; ">폴리백</small>
                                                    </div>
                                                <?php } ?>
                                            </td>
                                        <?php } //주문번호 ?>
                                        ...
                                        <?php if($gridKey === 'orderName'){ //주문자 ?>
                                            <td class="js-member-info" data-member-no="<?= $val['memNo'] ?>" data-member-name="<?= $val['orderName'] ?>" data-cell-phone="<?= $val['smsCellPhone'] ?>">
                                                ...
                                                <p class="mgb0">
                                                    ...
                                                    <?php } else { ?>
                                                        <?php if (!$isProvider) { ?>
                                                            <button type="button" class="btn btn-link font-eng js-layer-crm" data-member-no="<?= $val['memNo'] ?>">(<?= $memberMasking->masking('order','id',$val['memId']); ?>/<?=$val['nickNm']?>)
                                                        <?php } else { ?>
                                        ...
                                        <?php if($gridKey === 'orderGoodsNm'){ //주문상품 ?>
                                            <td class="text-left">
                                                ...
                                                <?php if($val['sOrderNo'] > 0 && 6 == $val['scmNo'] ) {?>
                                                <div>
                                                    <small class="text-danger">★ 한국타이어 본사 주문건 ★</small>
                                                    <?php if('0000-00-00' !== $val['reqDeliveryDt'] ) {?>
                                                    <br><small class="sl-blue">배송요청일 : <?=$val['reqDeliveryDt']?></small>
                                                    <?php } ?>
                                                    <?php if($val['giftAmount'] > 0 ) {?>
                                                        <br><small class="text-muted">적립금 : <?=number_format($val['giftAmount'])?>원</small>
                                                    <?php } ?>
                                                </div>
                                                <?php }?>
                                            </td>
...
<?php include '_order_list_script.php' ?>
