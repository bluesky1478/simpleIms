...
<script type="text/javascript">
    ...
    $(function(){
        ...
        <?php
            if( !empty($claimSno) ){
        ?>
            var goodsSnoList = '<?=$orderGoodsSnoList?>'.split(',');
            var reqCntList = '<?=$reqCnt?>'.split(',');

            for(var key in goodsSnoList){
                var goodsNo = goodsSnoList[key];
                var reqCnt = reqCntList[key];
                //체크
                $('input[name="bundle[orderCheck]['+goodsNo+']"]').prop('checked',true);
                $('input[name="refund[statusCheck]['+goodsNo+']"]').prop('checked',true);
                $('input[name="back[statusCheck]['+goodsNo+']"]').prop('checked',true);

                $('select[name="exchangeCnt['+goodsNo+']"]').val(reqCnt);
                $('select[name="refund[goodsCnt]['+goodsNo+']"]').val(reqCnt);
                $('select[name="back[goodsCnt]['+goodsNo+']"]').val(reqCnt);
            }
        <?php
            }
        ?>
