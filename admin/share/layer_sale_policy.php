        ...
        <div class="search-detail-box">
            <table class="table table-cols">
                ...
                <tr>
                    ...
                    <td>
                        <div class="form-inline">
                            본사 : <input type="text" name="companyRatio" class="form-control number width20p" id="addCompanyRatio" @keyup="inCompanyRatio"  /> %
                            / 구매자 : <input type="text" name="buyerRatio" class="form-control number  width20p" readonly="readonly"  id="addBuyerRatio"  /> %
                        </div>
                    </td>
                </tr>
        ...
        <div class="js-field-select-wapper">
            <table class="table table-rows">
                ...
                <tr>
                    ...
                    <th class="width20p">정책명</th>
                    <th class="width5p">본사%</th>
                    <th class="width5p">구매자%</th>
                    <!--<th class="width10p">사용여부</th>-->
                ...
                <tr v-for="(item, index) in items" :key="index">
                    ...
                    <td class="center">
                        <input type="text" name="companyRatio" :value="item.companyRatio" :data-sno="item.sno" class="form-control center" @keyup="updateInputPolicy" :id="item.companyRatioId" readonly />
                    </td>
                    <td class="center">
                        <input type="text" name="buyerRatio" :value="item.buyerRatio" :data-sno="item.sno" class="form-control center" @keyup="updateInputPolicy" :id="item.buyerRatioId" readonly="readonly" />
                    </td>
                    <!--<td class="center">
                    </td>-->
                    <!--<td class="center">
...
<script type="text/javascript">
	...
	$(document).ready(function () {
	    var appPolicyList = new Vue({
            ...
            , methods : {
                inCompanyRatio : function() {
                    $('#addBuyerRatio').val(100 - Number($('#addCompanyRatio').val()));
                }
                , addPolicy : function(){
                    $("input[name='mode']").val('add_sale_policy');
                ...
                , updateInputPolicy : function(e){
                    var sno = e.target.attributes.getNamedItem('data-sno').value;
                    var updateData = {
                        sno : sno
                        , updateKey : e.target.name
                        , updateValue : e.target.value
                        , updatePolicyType : 'sale' // freeSale, sale, survey
                    };
                    updatePolicy(updateData);

                    if( 'companyRatio' === e.target.name ){
                        for(var key in this.items){
                            var buyerRatio = 100 - Number($('#companyRatioId_' + this.items[key].sno).val());
                            $('#buyerRatioId_' + this.items[key].sno).val(buyerRatio);
                        }
                    }
                }
        ...
        var setPolicyItemList = function(){
            $.post('goods_ps.php', 'mode=get_sale_policy_list', function (data) {
                ...
                for(var key in parsingData){
                    ...
                    parsingData[key]['buyerRatioId'] = 'buyerRatioId_' + parsingData[key]['sno'];
                    parsingData[key]['companyRatioId'] = 'companyRatioId_' + parsingData[key]['sno'];
                }
